...
def finetune_cbf(v: MLP, q: MLP, pi: MLP, X: np.ndarray, vstar: np.ndarray,
                 model, d_grid: np.ndarray, gamma: float, control_max: float,
                 margin: float, epochs: int, batch: int, lr: float,
                 lip_lambda: float = 1e-4, anchor_w: float = 0.1,
                 cons_w: float = 1.0, pi_anchor: np.ndarray | None = None,
                 pi_anchor_w: float = 0.5, pi_lip_lambda: float = 0.0,
                 seed: int = 0, verbose: bool = True) -> dict:
    """Co-train (V, Q, pi) into a verifiable robust CBF (the Gate-D fix).

    On states X in {V>=0}, minimise:
      decrease   relu(gamma V(x) - min_k Q(x, clip(pi(x)), d_k) + margin)
                 -- the fallback achieves V(x+) >= gamma V(x) robustly (C3);
      consistency (Q(x,u,d) - V(f(x,u,d)))^2  on random (u,d)
                 -- Q ~ V o f, so C3 reflects the TRUE decrease and Q inherits
                    V's heading-selectivity (low Q for set-leaving u);
      anchor      anchor_w (V(x) - V*(x))^2  -- keep V near the teacher (valid
                    {V>=0}, no collapse);
      Lipschitz   lip_lambda ||W||^2 for V and Q  -- LOW Lipschitz so the CROWN
                    bound on the C3 decrease is tight (no holes in {V>=0}),
                    which lets the *fallback-only* C2 (qcbf.certify.refine.
                    c2_fallback_fixed_point) close without the antecedent skip.

    Gate-D escape: a low-Lipschitz CBF is CROWN-tight (hole-free C3) but flat,
    so it cannot fire the all-actions skip -- which is exactly why the
    certificate uses the fallback-only invariance argument here.
    """
    n = len(X)
    if n == 0:
        if verbose:
            print("  [cbf] skipped: empty {V>=0} training set")
        return {}
    rng = np.random.default_rng(seed)
    optV, optQ, optPi = Adam(v, lr=lr), Adam(q, lr=lr), Adam(pi, lr=lr)
    K = len(d_grid)
    hist = {}
    for ep in range(epochs):
        order = rng.permutation(n)
        tot_dec = tot_cons = tot_anch = 0.0
        cnt = 0
        for s in range(0, n, batch):
            idx = order[s:s + batch]
            xb = X[idx]
            ...
            # ---- forward V(x) (cached) and pi(x) ----------------------- #
            Vx, zsV, hsV = v.forward(xb, cache=True)
            Vx = Vx.ravel()
            y, zsP, hsP = pi.forward(xb, cache=True)
            y = y.ravel()
            u = np.clip(y, -control_max, control_max)

            # ---- decrease hinge: min_d Q(x, u, d_k) vs gamma V --------- #
            qvals = np.empty((B, K))
            for k, dk in enumerate(d_grid):
                z = np.concatenate([xb, u[:, None], np.full((B, 1), dk)], axis=1)
                qvals[:, k] = q(z).ravel()
            kmin = np.argmin(qvals, axis=1)
            m = qvals[np.arange(B), kmin]
            slack = gamma * Vx - m + margin
            active = slack > 0.0
            tot_dec += float(np.sum(np.maximum(slack, 0.0)))

            # grad of decrease wrt V (through V at xb): +gamma * active
            dV_dec = gamma * active.astype(np.float64) / B
            # grad wrt Q at (x,u,d_kmin): -active   (push min_d Q up)
            dmin = d_grid[kmin]
            zmin = np.concatenate([xb, u[:, None], dmin[:, None]], axis=1)
            outQm, zsQm, hsQm = q.forward(zmin, cache=True)
            dQ_dec = (-active.astype(np.float64) / B)[:, None]
            # grad wrt u (through Q), routed to pi through the clip
            gz = q.input_grad(zmin, np.ones((B, 1)))
            dL_du = np.where(active, -gz[:, 3], 0.0) / B
            sat_hi = (y >= control_max) & (dL_du < 0)
            sat_lo = (y <= -control_max) & (dL_du > 0)
            dL_dy = np.where(sat_hi | sat_lo, 0.0, dL_du)
            # anchor pi to the oracle robust-greedy labels so the decrease
            # nudge cannot saturate it into a degenerate (bang-bang) policy
            if pi_anchor is not None:
                dL_dy = dL_dy + pi_anchor_w * 2.0 * (y - pi_anchor[idx]) / B

            # ---- Q<->V o f consistency on random (u, d) ---------------- #
            ur = rng.uniform(-control_max, control_max, B)
            dr = rng.uniform(d_grid[0], d_grid[-1], B)
            zc = np.concatenate([xb, ur[:, None], dr[:, None]], axis=1)
            outQc, zsQc, hsQc = q.forward(zc, cache=True)
            nxt = model.step(xb, ur, dr)
            Vf = v(nxt).ravel()                       # V frozen for this term
            resid = outQc.ravel() - Vf
            ...
            # ---- accumulate + Adam steps ------------------------------ #
            gWv, gbv, _ = v.backward(zsV, hsV, (dV_dec + dV_anch)[:, None])
            gWqm, gbqm, _ = q.backward(zsQm, hsQm, dQ_dec)
            gWqc, gbqc, _ = q.backward(zsQc, hsQc, dQ_cons)
            gWp, gbp, _ = pi.backward(zsP, hsP, dL_dy[:, None])
            # Lipschitz / weight penalty (V and Q)
            for i in range(len(v.W)):
                gWv[i] = gWv[i] + lip_lambda * 2.0 * v.W[i]
            gWq = [gWqm[i] + gWqc[i] + lip_lambda * 2.0 * q.W[i]
                   for i in range(len(q.W))]
            gbq = [gbqm[i] + gbqc[i] for i in range(len(q.b))]
            if pi_lip_lambda > 0:                  # smoother pi -> tighter
                for i in range(len(pi.W)):         # fallback successor box
                    gWp[i] = gWp[i] + pi_lip_lambda * 2.0 * pi.W[i]
            optV.step(gWv, gbv)
            optQ.step(gWq, gbq)
            optPi.step(gWp, gbp)

        if verbose and (ep % max(1, epochs // 10) == 0 or ep == epochs - 1):
            print(f"  [cbf] ep {ep:3d}  decr_hinge={tot_dec/cnt:.4f}  "
                  f"cons_mse={tot_cons/cnt:.4f}  anchor_mse={tot_anch/cnt:.4f}")
        hist = {"decr_hinge": tot_dec / cnt, "cons_mse": tot_cons / cnt,
                "anchor_mse": tot_anch / cnt}
    return hist
