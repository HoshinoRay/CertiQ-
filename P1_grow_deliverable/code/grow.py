...
class GrowEngine:
    ...
    def __init__(self, grid: Grid4D, seed3d, K3d, V03d, menu, encode_fn):
        self.g = grid
        ...
        self.K3d = K3d.astype(bool) | self.seed3d  # safe region g>=0 (seed in it)
        self.V03d = V03d.astype(bool) | self.seed3d  # B1 anchor {lb V_theta>=0}
        VK3d = self.K3d & self.V03d
        self.VK3d = VK3d
        # lift to 4-D (heading-independent masks): seed/K/V0 do not depend on psi
        ...
        self.K = np.broadcast_to(self.K3d[:, :, None, :], grid.shape).copy()
        self.V0 = np.broadcast_to(self.V03d[:, :, None, :], grid.shape).copy()
    ...
    def _precompute(self, ci, cj, ck, cl, verbose=True):
        ...
        PK = _prefix_sum(self.K)                  # for failure attribution
        PV0 = _prefix_sum(self.V0)
        nc, na = len(ci), len(self.menu)
        store = []
        # per-candidate aggregates (OR across actions) for the failure breakdown
        any_dom = np.zeros(nc, bool)              # >=1 action stays in the grid
        any_domK = np.zeros(nc, bool)             # >=1 action: touched subset K
        any_valid = np.zeros(nc, bool)            # >=1 action: touched subset K n V_0
        ...
        for a, (acmd, dcmd) in enumerate(self.menu):
            ...
            def _touch(P):                            # box-sum over (possibly wrapped) psi
                s = _boxsum(P, i0, i1, j0, j1, kA0, kA1, l0, l1)
                return s + np.where(kB_act, _boxsum(
                    P, i0, i1, j0, j1, np.where(kB_act, kB0, 0),
                    np.where(kB_act, kB1, 0), l0, l1), 0)

            inK = dom & (_touch(PK) == boxvol)
            inV0 = dom & (_touch(PV0) == boxvol)
            valid = dom & (_touch(PVK) == boxvol)     # = inK & inV0 (& dom)
            any_dom |= dom
            any_domK |= inK
            any_valid |= valid
        ...
        if pb is not None:
            pb.done()
        self._agg = dict(any_dom=any_dom, any_domK=any_domK, any_valid=any_valid)
        return store
    ...
    def _blocked_breakdown(self, ci, cj, ck, cl, store, not_added):
        """Proven failure attribution for candidates never added, by PRIORITY of
        the most-recoverable reason a menu action exists for:

          frontier_blocked : >=1 action keeps Enc subset K n V_0 (valid) but its
                              successor touches a non-R cell -> capture-basin
                              frontier the lfp could not reach (most recoverable);
          v0_blocked       : no valid action, but >=1 keeps Enc subset K (in
                              domain & collision-free) -> the learned anchor
                              {lb V_theta>=0} is the binding constraint;
          k_blocked        : no in-K action, but >=1 stays in the grid domain ->
                              every reachable successor risks g<0 (near obstacle);
          domain_blocked   : no action keeps Enc inside the grid (boundary).

        The four are mutually exclusive and sum to n_not_added.  The
        v0_blocked share is exactly the V_theta-gate diagnostic (it is ~0 for the
        analytic ideal whose V_0 = Omega*)."""
        nb = int(not_added.sum())
        if nb == 0:
            return {"frontier_blocked": 0, "v0_blocked": 0, "k_blocked": 0,
                    "domain_blocked": 0}
        ad = self._agg
        frontier = not_added & ad["any_valid"]
        v0 = not_added & ~ad["any_valid"] & ad["any_domK"]
        kb = not_added & ~ad["any_domK"] & ad["any_dom"]
        dom = not_added & ~ad["any_dom"]
        return {"frontier_blocked": int(frontier.sum()),
                "v0_blocked": int(v0.sum()),
                "k_blocked": int(kb.sum()),
                "domain_blocked": int(dom.sum())}

    # ------------------------------------------------------------------ #
    def compute_aver(self, verbose=True):
        """Saturation table A_ver(c) = {u in U_menu : Enc(c,u) subset R_inf} for
        every cell of the converged R_inf (Lemma 5.1).  Returns a 4-D uint32 grid
        of action bitmasks (bit a set iff menu[a] keeps the successor in R_inf)
        plus per-cell counts.  This is the deployable certified shield: any action
        in A_ver(c) keeps R_inf invariant; the runtime then picks among them."""
        ci, cj, ck, cl = self._candidate_cells()        # all K n V_0 \ seed cells
        # A_ver only matters on R_inf cells; restrict to grown + seed cells
        Rci, Rcj, Rck, Rcl = np.nonzero(self.R)
        gx, gy, gp, gv = self.g.pxe, self.g.pye, self.g.pse, self.g.ve
        lo = np.column_stack([gx[Rci], gy[Rcj], gp[Rck], gv[Rcl]])
        hi = np.column_stack([gx[Rci + 1], gy[Rcj + 1], gp[Rck + 1], gv[Rcl + 1]])
        P = _prefix_sum(self.R)
        aver = np.zeros(self.g.shape, dtype=np.uint32)
        count = np.zeros(self.g.shape, dtype=np.int16)
        pb = Progress(len(self.menu), "A_ver") if verbose else None
        for a, (acmd, dcmd) in enumerate(self.menu):
            nx_lo, nx_hi, ny_lo, ny_hi, np_lo, np_hi, nv_lo, nv_hi = \
                self.encode_fn(acmd, dcmd, lo, hi)
            i0, i1, dom_x = self._idx_ranges(nx_lo, nx_hi, gx)
            j0, j1, dom_y = self._idx_ranges(ny_lo, ny_hi, gy)
            l0, l1, _ = self._idx_ranges(nv_lo, nv_hi, gv)
            k0r, k1r, _ = self._idx_ranges(np_lo, np_hi, gp, periodic=True)
            kspan = np.minimum(k1r - k0r + 1, self.npsi)
            full = (k1r - k0r + 1) >= self.npsi
            ka0 = np.mod(k0r, self.npsi); ka1 = np.mod(k1r, self.npsi)
            ...
            kA0 = np.where(full, 0, ka0)
            kA1 = np.where(full, self.npsi - 1, np.where(wrap, self.npsi - 1, ka1))
            kB1 = np.where(wrap, ka1, -1); kB0 = np.zeros_like(kB1); kB_act = kB1 >= kB0
            boxvol = ((i1 - i0 + 1) * (j1 - j0 + 1) * kspan * (l1 - l0 + 1)).astype(np.int64)
            bs = _boxsum(P, i0, i1, j0, j1, kA0, kA1, l0, l1)
            bs = bs + np.where(kB_act, _boxsum(P, i0, i1, j0, j1,
                               np.where(kB_act, kB0, 0), np.where(kB_act, kB1, 0),
                               l0, l1), 0)
            ok = (dom_x & dom_y) & (bs == boxvol)
            sel = np.flatnonzero(ok)
            aver[Rci[sel], Rcj[sel], Rck[sel], Rcl[sel]] |= np.uint32(1 << a)
            count[Rci[sel], Rcj[sel], Rck[sel], Rcl[sel]] += 1
            if pb is not None:
                pb.update(a + 1)
        if pb is not None:
            pb.done()
